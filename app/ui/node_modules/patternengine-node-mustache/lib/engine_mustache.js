...
//class-like factory for instantiating partial objects
function PartialObj(key, params, content) {
  this.key = key;
  this.partial = engine_mustache.findPartialKey(key);
  this.params = params;
  this.content = '';
  ...
  if (params) {
    this.content = engine_mustache.renderParams(content, params);
  } else {
    this.content = content;
  }
  this.content = this.content.replace(/(>|\})\s+(<|\{)/g, '$1 $2').replace(/\s*\n/g, ''),

  // escape the parametered tags within partials by changing delimiters to unicodes
  // for start-of-text and end-of-text
  this.nestedDataKeys = this.content.match(/\{\{#[\S\s]+?\}\}/g) || [];
  this.nestedDataKeys = this.nestedDataKeys.concat(this.content.match(/\{\{\^[\S\s]+?\}\}/g) || []);
}
...
var engine_mustache = {
  ...
  newPartialObj: function (key, params, content) {
    return new PartialObj(key, params, content);
  },
  ...
  registerPartial: function (key, pattern, patternlab, getPartial) {
    ...
    // check if the key no styleModifiers or params
    // if that's the case, copy the partialObj from the pattern
    keyParts = key.match(utilMustache.partialKeyRE);
    if (keyParts[1] && !keyParts[2] && !keyParts[3]) {
      partialStr = keyParts[1].trim();
      patternPartial = getPartial(partialStr, patternlab);
      partialObj = JSON.parse(patternPartial.partialObj);
      partialObj.key = key;
      partialObj.partial = partialStr;
      // save to patternlab object
      patternlab.partials[key] = partialObj;

    // else create a new partialObj with the factory
    } else {
      ...
      partialObj = new PartialObj(key, params, pattern.template);
